...
import { setactivarchivtab } from '../redux/actions/navActions';
...
function GymFem({ setactivarchivtab, activeTab, entrainements, benevoles }) {
  const classes = useStyles();
  const theme = useTheme();

  function handleChange(event, newValue) {
    setactivarchivtab(newValue);
  }

  function handleChangeIndex(index) {
    setactivarchivtab(index);
  }
...
const mapStateToProps = state => ({ activeTab: state.activarchivtab });
